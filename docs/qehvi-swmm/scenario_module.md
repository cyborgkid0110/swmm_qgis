...
The `src.scenario` package is the single home for every operation that reads or writes SWMM `.inp` scenario files. The qEHVI-SWMM pipeline classes (`InputqEHVISWMM`, `KPIEvaluation`) delegate to this package so that scenario semantics are not duplicated across Step 1 and Step 2.
