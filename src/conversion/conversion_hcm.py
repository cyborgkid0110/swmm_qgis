"""
SWMM Conversion - Ho Chi Minh City (HCM) Region

Generates SWMM .inp file for features within the HCM region.
Region: canals + congdap combined extent (106.36-106.98°E, 10.38-11.16°N).
...
# Combined extent of canals + congdap datasets in WGS84
HCM_BBOX = (106.364036, 10.382483, 106.977959, 11.158576)


def run_hcm():
    dataset_dir = os.path.join(REPO_DIR, "dataset")
    result_dir = os.path.join(REPO_DIR, "result", "swmm_output")
    inp_file = os.path.join(result_dir, "hcm_model.inp")
    ...
    conv = Conversion(dataset_dir=dataset_dir, result_dir=result_dir,
                      bbox=HCM_BBOX, dem_path=dem_path)

    print("=" * 60)
    print("SWMM Conversion - HCM Region")
    print(f"  BBOX: {HCM_BBOX}")
    print("=" * 60)
    ...
    options_path = conv.create_options_table(
        os.path.join(result_dir, "hcm_options.xlsx")
    )
